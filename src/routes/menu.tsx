...
const MANDI: Dish[] = [
  { name: "Chicken Kuzhi Mandi", img: dishChicken, tag: "Signature", portions: [
    { label: "Quarter", price: 180 }, { label: "Half", price: 340 },
    { label: "Half + Qtr", price: 520 }, { label: "Full", price: 660 },
  ]},
  { name: "Alfaham Mandi", img: dishAlfaham, tag: "Popular", portions: [
    { label: "Quarter", price: 200 }, { label: "Half", price: 360 },
    { label: "Half + Qtr", price: 560 }, { label: "Full", price: 700 },
  ]},
  { name: "Periperi Alfaham Mandi", img: dishPeri, tag: "Spicy", portions: [
    { label: "Quarter", price: 200 }, { label: "Half", price: 370 },
    { label: "Half + Qtr", price: 570 }, { label: "Full", price: 720 },
  ]},
  { name: "Honey Chilly Alfaham Mandi", img: dishPeri, portions: [
    { label: "Quarter", price: 210 }, { label: "Half", price: 370 },
    { label: "Half + Qtr", price: 580 }, { label: "Full", price: 720 },
  ]},
  { name: "Pepper Alfaham Mandi", img: dishAlfaham, portions: [
    { label: "Quarter", price: 200 }, { label: "Half", price: 370 },
    { label: "Half + Qtr", price: 570 }, { label: "Full", price: 720 },
  ]},
  { name: "Kandhari Alfaham Mandi", img: dishPeri, portions: [
    { label: "Quarter", price: 200 }, { label: "Half", price: 370 },
    { label: "Half + Qtr", price: 570 }, { label: "Full", price: 720 },
  ]},
  { name: "Beef Mandi", img: dishBeef, tag: "Chef's Pick", portions: [
...
const SECTIONS: { key: SectionKey; label: string; sub: string; Icon: typeof Drumstick; count: number }[] = [
  { key: "mandi", label: "Mandi Dishes", sub: "Slow-cooked Arabian classics", Icon: Drumstick, count: MANDI.length },
  { key: "pieces", label: "Pieces Only", sub: "Just the meat, no rice", Icon: Beef, count: PIECES.length },
  { key: "juice", label: "Juice", sub: "Fresh & chilled", Icon: GlassWater, count: JUICES.length },
];
    ...
    <div className="min-h-screen bg-background text-foreground pb-32">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-xl bg-background/70 border-b border-border/60">
        <div className="max-w-3xl mx-auto px-5 py-4 flex items-center justify-between">
          <Link to="/" className="w-10 h-10 grid place-items-center rounded-full border border-border hover:bg-secondary transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="text-center">
            <div className="font-display text-xl tracking-[0.25em]">MENU</div>
            <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Saud Kuzhimandhi</div>
          </div>
          <a href="tel:+917902655501" className="w-10 h-10 grid place-items-center rounded-full gold-grad text-black">
        ...
        <div className="max-w-3xl mx-auto px-5 pt-8 pb-6">
          <div className="relative rounded-3xl overflow-hidden border border-border">
            ...
            <div className="absolute inset-0 p-6 flex flex-col justify-end">
              <div className="text-xs uppercase tracking-[0.35em] text-accent font-semibold">Our Menu</div>
              <h1 className="font-display text-5xl leading-none mt-1 text-white">
                AUTHENTIC <span className="text-gold">TASTE</span>
              </h1>
              <div className="mt-2 flex items-center gap-2 text-white/70 text-xs">
                <span className="h-px w-6 bg-accent" /> Wood-fired · Hand-crafted
              </div>
            </div>
          </div>
        </div>
          ...
          {SECTIONS.map(({ key, label, Icon, count }) => {
            ...
            return (
              <button
                ...
                className={`group relative rounded-2xl border p-4 flex flex-col items-center gap-2 transition-all ${
                  isActive
      ...
      <section className="max-w-3xl mx-auto px-5 mt-8">
        ...
        {active === "juice" ? (
          <div className="grid grid-cols-2 gap-3">
            {JUICES.map((j) => (
              <div key={j.name} className="flex items-center justify-between gap-3 rounded-2xl border border-border bg-card px-4 py-4">
                <div>
                  <div className="font-semibold">{j.name}</div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-0.5">Fresh</div>
                </div>
          ...
          <div className="space-y-4">
            {(active === "mandi" ? MANDI : PIECES).map((d) => (
              <article key={d.name} className="rounded-3xl border border-border bg-card overflow-hidden card-shadow">
                <div className="flex gap-4 p-3">
                  {d.img && (
                    <div className="shrink-0 w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden">
        ...
        <div className="rounded-full gold-grad text-black flex items-center justify-between pl-5 pr-2 py-2 shadow-2xl">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              ...
              <div className="text-[10px] opacity-80">Get it delivered hot!</div>
            </div>
          </div>
          <OrderNowButton
            className="w-12 h-12 grid place-items-center rounded-full bg-black text-accent"
